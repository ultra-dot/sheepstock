            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-6">
                ...
                <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl border border-emerald-500/10 rounded-3xl shadow-xl overflow-hidden glass-card">
                    ...
                    <div className="md:hidden divide-y divide-emerald-500/5">
                        {filtered.length === 0 ? (
                            <div className="text-center py-10 text-slate-500 font-medium text-sm">Belum ada rekam medis.</div>
                        ) : (
                            filtered.map((record) => (
                                <div key={record.id} className="p-4 space-y-3 hover:bg-emerald-500/5 transition-colors">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 font-bold rounded text-xs">{record.livestocks?.qr_code || '-'}</span>
                                            <span className="text-[10px] text-slate-400 font-semibold whitespace-nowrap">{new Date(record.date).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: '2-digit' })}</span>
                                        </div>
                                        {statusBadge(record.status)}
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-xs text-slate-600 dark:text-slate-400 font-medium"><span className="text-[10px] text-slate-400 uppercase font-bold mr-1">Keluhan:</span>{record.illness_description}</p>
                                        <p className="text-xs text-slate-600 dark:text-slate-400 font-medium"><span className="text-[10px] text-slate-400 uppercase font-bold mr-1">Tindakan:</span>{record.treatment}</p>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            {record.inventory_items?.name ? (
                                                <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-bold rounded-full border border-slate-200 dark:border-slate-700">
                                                    <Pill className="w-3 h-3" />{record.inventory_items.name}
                                                </span>
                                            ) : <span className="text-slate-400 text-[10px]">Tanpa obat</span>}
                                            {record.resolved_at ? (
                                        ...
                                        <div className="flex items-center gap-1">
                                            <button onClick={() => setViewRecord(record)} className="text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors cursor-pointer p-1" title="Detail">
                                            ...
                                            <button onClick={() => handleDelete(record.id)} className="text-slate-400 hover:text-rose-500 transition-colors cursor-pointer p-1" title="Hapus">
